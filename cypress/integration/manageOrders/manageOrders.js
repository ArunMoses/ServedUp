...
import { cardPaymentiFrame, cardNumber, expiryDate, cvc, cardError } from "../../locators/checkout";
...
And('I enter valid card details', function () {
  ...
  cy.getWithinIframe(cardNumber()).type(this.cardDetails.valid.cardNumber)
...
And('I enter invalid card details', function () {
  cy.get('@wrapper').find(cardPaymentiFrame(), { timeout: 5000 })
  cy.getWithinIframe(cardNumber()).type(this.cardDetails.invalid.cardNumber);
...
And('I enter expired card details', function () {
  cy.get('@wrapper').find(cardPaymentiFrame(), { timeout: 5000 })
  cy.getWithinIframe(cardNumber()).type(this.cardDetails.expired.cardNumber);
...
And('I enter card details with incorrect cvc', function () {
  cy.get('@wrapper').find(cardPaymentiFrame(), { timeout: 5000 })
  cy.getWithinIframe(cardNumber()).type(this.cardDetails.incorrect_cvc.cardNumber);
...
Then('My payment should be successful', function () {
  const expectedPaymentSuccessHeader = 'Thank you';
    cy.xpath(paymentSuccessHeader(), { timeout: 8000 }).then((paymentSuccessHeader)=>{
      // assert header
    ...
    cy.xpath(orderConfirmationMobile()).then((mobile)=>{
      // assert mobile
        expect(mobile.text()).to.contain(`+44${this.personalInfo.mobile.substring(1)}`)
    })
    cy.get('@wrapper').find(orderConfirmationSubTotal()).then((total)=>{
...
Then('I should see {string} error', (error) => {
  cy.get('@wrapper').find(cardError(), { timeout: 8000 }).should('be.visible').then((cardError)=>{
    // assert error
